...
\begin{enumerate}[label=\emph{\arabic*})]
	...
	\item \textit{Antal unika kunder}:  \Sexpr{ Format(Summary[, AntalPerson] )  } st. %% change
	\item \textit{Inflöde av pengar}:  \Sexpr{ Format(Summary[, MoneyIN] ) } SEK, är beräknad genom inflöde av likvida medel minus utflödet av pengar. %% change
		Sker på grund av att vi ibland måste returnera pengar till slutkunden. 
	\item \textit{Charges}: Är summan av alla intäkter från inkasso, ränta betalningspåminnelse, transaktionsavgift och fakturaavgift, vilket blir \Sexpr{ Format(Summary[, Charges]   ) }  SEK. %%change
	\item \textit{Förväntad förlust}: Är alla fakturor som idag är ansedda som en kredit förlust och avser endast själva kapital beloppet,  \Sexpr{ Format(Summary[, AmountLoss] ) }  SEK. %%change
...
\begin{enumerate}[label=\emph{\arabic*})]
	\item \textit{Icke linjära relationer}, 
	\item \textit{Robust mot korrelationer mellan variablar, distributioner och NA värden},
	\item \textit{Enkel att förstå},
	\item \textit{Snabbhet både att träna och prognos}
\end{enumerate}

Nackdelarna är . 
\begin{enumerate}[label=\emph{\arabic*})]
	\item \textit{Dålig accuarcy och project}, bra inom det data set som den tränas inom men inte utanför det.
	\item \textit{Ineffektiv för linjära förhållanden}
\end{enumerate}
...
Skulle vi exempelvis tillämpa regel 1, så skulle vi bespara företaget en kreditförlust på \Sexpr{  Format(Tree1[, CreditLoss]) } SEK
(eftersom vi då köpt fakturor för den summa), men eftersom vi tillämpar regeln så kommer vi att förlora en förväntad intäkt\footnote{
	Alla är ju inte dåliga kunder, ca  \Sexpr{  Tree1[, 100- RatioCL] } äv kunderna är ansedda som bra.
},  \Sexpr{ Format(Tree1[, Profit])}   SEK. På samma viss är det för regel två, där vi köper fakturor från folk som har 
en mindre skuld än 226 SEK. På så viss har vi en kreditförlust på \Sexpr{  Format(Tree2[, CreditLoss]) } men en 
intäkt på \Sexpr{ Format(Tree2[, Profit])} SEK.
...
\begin{table}[ht]
\centering
\caption{ Prognos av modell enligt fig~\eqref{fig:tree1} på test data antal \Sexpr{ SummaryTest[, Antal] } }
\setlength{\extrarowheight}{2pt}
\begin{tabular}{l>{\sffamily}l c }
\toprule 
Prognos/Utfall &  False & True    \\
\cmidrule(l){2-3}
	False & \Sexpr{ conFTree$table['False', 'False']  }  & \Sexpr{ conFTree$table['False', 'True'] } \\
	 True & \Sexpr{  conFTree$table['True', 'False'] }  & \Sexpr{ conFTree$table['True', 'True']}   \\
	 Accuracy & \Sexpr{  round(conFTree$overall[['Accuracy']], 3) } & -- \\
	 Sensitivity & 	 \Sexpr{  round(conFTree$byClass[['Sensitivity']], 3) } & -- \\
	  Specificity & 	 \Sexpr{  round(conFTree$byClass[['Specificity']], 3) } & -- \\
	  Precision (PPV)& 	 \Sexpr{  round(conFTree$byClass[['Pos Pred Value']] , 3) } & -- \\
	  Neg predictive value (NPV)& 	 \Sexpr{  round(conFTree$byClass[['Neg Pred Value']] , 3) } & -- \\
	  AUC	& \Sexpr{round(aucTree, 3)} & --\\
\bottomrule
\end{tabular}
\label{Q:roc}
\end{table}
...
Boosting gets a better test error relative to bagging. \textit{AdaBoost} builds an additive logistic regression
model by stagewise fitting. see statistical learning pp 343
